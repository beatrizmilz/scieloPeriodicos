...
dados <-
  dplyr::left_join(periodicos_ativos, instrucoes, by = "id_periodico") |>  dplyr::mutate(
    Periodico = glue::glue("<a href='{url_instrucoes}' target='_blank'>{titulo_periodico}</a>"),
...
dados_filtrados |> 
  dplyr::select(Periodico, 
                ...
                Texto = texto_secao) |> 
  reactable::reactable(sortable = TRUE, searchable = TRUE, defaultColDef = reactable::colDef(html = TRUE))
```
